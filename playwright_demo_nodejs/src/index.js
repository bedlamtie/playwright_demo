...
(async () => {
    ...
    let sleepTime
    while (!sleepTime) {
      sleepTime = await consola.prompt('Enter Sleep Time (seconds): ', {
        initial: '2000'
      });
    ...
    let startSn;
    while (!startSn) {
      startSn = await consola.prompt('Enter Start SN: ', {
        initial: 'w5021927ka0'
      });
    ...
    let searchCount;
    while (!searchCount) {
      searchCount = await consola.prompt('Enter Search Count: ', {
        initial: '10'
      });
    ...
    const frame = await page.frameLocator('#eh-page iframe').owner().contentFrame();
